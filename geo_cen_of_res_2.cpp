...
void geo_cen_of_side_chain(vector<naa> &w_c3,vector <aa> &w_c2,vector<aa> w_c,char asa[])
{
        aa amino;
	...
	setprecision(3)<<setw(8)<<c_x<<setprecision(3)<<setw(8)<<c_y<<setprecision(3)<<setw(8)<<c_z<<endl;
	...
	w_c3.push_back(amino_n);
	 ...
	 for(i=0;i<w_c3.size();i++)
	 {
	  ...
	  ofstream file("angle.txt",ios::out|ios::app);
           file<<setiosflags(ios::left)<<setw(8)
	       <<"pn/rn"
           ...
           file<<setiosflags(ios::left)<<setw(8)<<asa
