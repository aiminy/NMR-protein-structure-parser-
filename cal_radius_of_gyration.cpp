...
//void geo_cen_of_res(vector<naa> &w_c3,vector <aa> &w_c2,vector<residue> &a_w_c,vector<aa> w_c)
{
	...
	residue ave_aa;
        ...
        //cout<<setiosflags(ios::left)<<setw(10)<<w_c.size();
...
*/
	...
	for(i=0;i<a_w_c.size();i++)
	{
           ...
           double ave_radius_of_gyration=0;
          
         //cout<<a_w_c.size()<<endl;
          //cout<<w_c[1].res_num<<endl;
          //cout<<"name"<<a_w_c[1].res_id<<"name"<<endl;
          //cout<<"NUM"<<a_w_c[1].res_num<<"NUM"<<endl; 
         for(i=0;i<a_w_c.size();i++)
         {
          ave_radius_of_gyration=pow((a_w_c[i].ave_x-c_m_x),2)+pow((a_w_c[i].ave_y-c_m_y),2)+pow((a_w_c[i].ave_z),2);
          ave_radius_of_gyration=sqrt(ave_radius_of_gyration);
          a_w_c[i].distance=ave_radius_of_gyration;
         }
/*
         for(i=0;i<a_w_c.size();i++)
         {  
          cout<<setiosflags(ios::left)<<setw(10)<<a_w_c[i].res_id
              <<setiosflags(ios::left)<<setw(10)<<a_w_c[i].res_num
              <<setiosflags(ios::left)<<setw(10)<<a_w_c[i].distance<<endl;
         }//end of for
  */
        
          //ave_radius_of_gyration=sqrt(ave_radius_of_gyration);
      
          //cout<<setiosflags(ios::left)<<setw(10)<<total_aa
          //    <<setiosflags(ios::left)<<setw(10)<<a_w_c.size() 
          //    <<setiosflags(ios::left)<<setw(10)<<ave_radius_of_gyration<<endl;
          //a_w_c.clear();
